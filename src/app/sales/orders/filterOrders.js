...
import "@/styles/sales/customer.css"; // Reuse styling
...
export default function FilterOrdersModal({
    ...
    // Initialize selectedProductData from props if needed (if currentFilters has products, we might need their names)
    // This is a bit tricky if we only have IDs. 
    // Ideally, parent should pass selectedProductData too, or we rely on user searching again.
    // For now, let's assume if IDs are present, we might check productResults or just show ID labels if names missing.
    // Better approach: Receive selectedProductsData from parent if available.
