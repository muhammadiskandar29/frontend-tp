...
export default function OrderBroadcastModal({ onClose, onAdd }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log("📤 [ORDER BROADCAST] Normalized payload:", JSON.stringify(requestBody, null, 2));
            console.log("📤 [ORDER BROADCAST] Target produk:", requestBody.target.produk);
            console.log("📤 [ORDER BROADCAST] Target status_order:", requestBody.target.status_order);
            console.log("📤 [ORDER BROADCAST] Target status_pembayaran:", requestBody.target.status_pembayaran);

            const res = await fetch("/api/sales/order/broadcast", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Accept: "application/json",
            ...
            console.log("✅ [ORDER BROADCAST] Success:", json);
            ...
            console.error("❌ [ORDER BROADCAST] Error:", err);
    ...
    const getStatusPembayaranLabel = (value) => {
        return STATUS_PEMBAYARAN_MAP[value]?.label || value;
    };
            ...
            <div className="orders-modal-card" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "800px", maxHeight: "90vh" }}>
                ...
                <form onSubmit={handleSubmit} className="orders-modal-body" style={{ overflowY: "auto", flex: 1 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
                        {/* Filter Status Order (Single Select) */}
                        <div className="orders-field">
                            <label style={{ display: "block", marginBottom: "0.75rem", fontWeight: 600 }}>
                                Filter Status Order
                            </label>
                                    ...
                                    return (
                                        <button
                                            ...
                                            style={{
                                                padding: "0.5rem 0.75rem",
                                                borderRadius: "6px",
                                                border: `1px solid ${isSelected ? "#f1a124" : "#e5e7eb"}`,
                                                background: isSelected ? "#fffbf0" : "#fff",
                                                color: isSelected ? "#d98e1d" : "#374151",
                                                fontSize: "0.875rem",
                        ...
                        {/* Filter Status Pembayaran (Single Select) */}
                        <div className="orders-field">
                            <label style={{ display: "block", marginBottom: "0.75rem", fontWeight: 600 }}>
                    ...
                    <div style={{ marginTop: "1.5rem", padding: "1rem", background: "#f3f4f6", borderRadius: "8px", fontSize: "0.875rem", color: "#4b5563" }}>
                        <p style={{ margin: 0 }}>
                            <i className="pi pi-info-circle" style={{ marginRight: "0.5rem" }} />
                            Broadcast akan dikirim ke semua customer yang <strong>memenuhi semua kriteria</strong> filter di atas.
                            Jika tidak ada filter yang dipilih selain produk, maka akan dikirim ke <strong>semua customer yang membeli produk tersebut</strong>.
                        </p>
                    </div>

                    <div className="orders-modal-footer" style={{ marginTop: "2rem", paddingTop: "1rem", borderTop: "1px solid #e5e7eb", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                        <button
                            type="button"
                            onClick={onClose}
